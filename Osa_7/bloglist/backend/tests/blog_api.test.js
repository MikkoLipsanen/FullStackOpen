...
describe('when there is initially some blogs saved', () => {
    beforeEach(async () => {
        ...
        const loginRes = await api.post('/api/login').send(helper.initialUser)
    ...
    describe('addition of a new blog', () => {
        ...
        test('if token is not included, adding a new blog item fails and response has status code 401', async () => {
            const newBlog = {
                title: 'Blog post with no likes',
                author: 'Hank Williams',
                url: 'www.dontlikeme.com',
...
describe('when there is initially one user at db', () => {
    ...
    test('creation fails with proper statuscode and message if password is too short', async () => {
        ...
        assert(
            result.body.error.includes(
                'minimum length for password is 3 characters'
            )
        )
    ...
    test('if user does not contain username and password fields, response has status code 400', async () => {
        ...
        await api.post('/api/users').send(noUsernameNoPasswordUser).expect(400)
    })
})
